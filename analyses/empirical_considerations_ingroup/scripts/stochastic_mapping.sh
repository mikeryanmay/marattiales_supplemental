...
for sample_dir in ${sample_dirs[@]}; do
	if [ -f "${sample_dir}/tree_combined.nex" ] && [ -f "${sample_dir}/params_combined.log" ]; then
		...
		echo -e "-t Warning: no combined samples found in ${sample_dir}."
	fi
done
...
parallel --eta --jobs $ncores "Rscript src/stochastic_map_sim.R --data ${data} --samples {1} --output {2} --nsim ${nsim} --overwrite ${overwrite}" ::: ${sample_dirs_combined[@]} :::+ ${output_dirs_combined[@]}
